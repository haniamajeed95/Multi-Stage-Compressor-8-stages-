...
Ptot_tot_list = []   # Stage total-to-total pressure ratio   [-]
...
# Shaft diameter based on preliminary criterion   
        ...
        D_1_h_loc  = 2*r_1_h                                                    # Inducer hub diameter                                       [m]
        r_1_t_n    = r_1_h+0.15                                                 # Value of first attempt for impeller tip radius             [m]
        Error_r1t  = 1                                                         # Tip radius convergence error                                [-]
        while Error_r1t > 0.0001:
                ...
                tan_beta_1_t  = u_1_t/C_1_m                                      # Inlet impeller tip relative flow angle tangent             [-]
                D_1_t_new     = 2*(((m_f/rho_1)*tan_beta_1_t)/(pi*Omega*(1-(D_1_h_loc/(2*r_1_t))**2)))**(1/3)   # Analytical Mach-minimizing tip diameter   [m]
                r_1_t_n       = D_1_t_new/2                                      # New tip radius                                             [m]
                Error_r1t     = abs(r_1_t_n-r_1_t)/r_1_t                         # Tip radius convergence error                                [-]
        ...
        M_1_t    = w_1_t / a_1_t                                                # Inlet impeller tip relative Mach number                    [-]
        ...
        beta_tt_imp   = beta_tt                                          # Impeller total to total pressure ratio (= stage target, by construction of delta_h_0)   [-]   
...
# Impeller Blade Loading 
        ...
        Df          = 1-(w2/w_1_t)+(0.75*(delta_h_0/u_2**2))/((w_1_t/w2)*((Z/pi)*(1-(D_1_t/D_2))+(2*(D_1_t/D_2))))                  # Diffusion factor                                                         [-]
        delta_hbl   = 0.05*Df**2*u_2**2                                                                                        # Impeller blade loading loss                                              [J/kg]   
    
#Impeller Clearance (unshrouded, open tip) -- kept for comparison, not used in tot_loss for the shrouded scenario
        delta_r     = 0.003*D_2                                                                                                # Impeller clearance                                                       [m]                                                                  
        delta_hcl   = 0.6*(delta_r/b2)*cth2*sqrt((4*pi*(r_1_t**2-r_1_h**2)*cth2*C_1_m)/(b2*Neff*(r_2-r_1_t)*(1+rho_2/rho_1)))  # Impeller clearance loss (unshrouded)                                     [J/kg]

#Shrouded Impeller Labyrinth Seal Leakage
        eps_leak    = 0.0005                                                                                                   # Leakage gap between seal teeth and casing                                [m]
        N_leak      = 4                                                                                                       # Number of teeth inside the labyrinth seal                                [-]
        C_D_leak    = 0.8                                                                                                     # Labyrinth seal discharge coefficient (straight-through w/ grooves)       [-]
        delta_h_is  = c_p*T_01*((beta_tt**((k-1)/k))-1)                                                                        # Isentropic enthalpy rise across the seal (stage pressure ratio)          [J/kg]
        S_leak      = pi*D_1_t*eps_leak                                                                                       # Annular leakage flow area                                                [m^2]
        m_leak      = C_D_leak*S_leak*rho_1*sqrt(2*delta_h_is/N_leak)                                                          # Labyrinth seal leakage mass flow rate                                    [kg/s]
        delta_hcl_shrouded = m_leak*delta_h_0/m_f                                                                              # Shrouded impeller leakage loss                                           [J/kg]

#=======================================================================#
# Impeller Incidence Loss
#=======================================================================#
        D1_inc    = sqrt((D_1_t**2+(r_1_h*2)**2)/2)                          # Geometric average diameter                    [m]
        u1        = pi*D1_inc*N/60                                           # Peripheral speed at impeller inlet            [m/s]
        w1_inc    = sqrt(C_1**2+u1**2)                                       # Relative velocity at impeller inlet           [m/s]
        beta1     = degrees(atan(u1/C_1_m))                                  # Relative flow angle                           [deg]
        if i == 1:
                beta1_opt_mid = beta1                                        # Fixed design (optimum) blade angle, set once at stage 1   [deg]
        finc      = 0.5                                                     # Incidence friction loss coefficient           [-]
        delta_hin = finc*w1_inc**2*sin(radians(beta1-beta1_opt_mid))**2      # Impeller incidence loss                       [J/kg]

#Impeller Mixing
        eps_wake    = 0.1                                                                                                       # Wake fraction of the blade-to-blade space                               [-]
        b_star      = 0.95                                                                                                     # Ratio of vaneless diffuser inlet width to impeller exit width           [-]
        delta_hmx   = (1/(1+tan(alpha2)**2))*((1-eps_wake-b_star)/(1-eps_wake))**2*(c_2**2/2)                                  # Impeller mixing loss                                                     [J/kg]
        
    
# Impeller Friction (Jansen)
        ...
        Lb         = (pi/8)*(D_2-(D_1_t+D1h)/2-b2+2*Lax) * (2/(((cos(beta_1_t)+cos(beta_1_h))/2)*cos(radians(beta_2_inf))))     # Impeller blade length                                                    [m]
        ...
        w_bar      = (c_th1+w_1_t+c_2+2*w_1_h+3*w2)/8                                                                          # Average relative velocity inside the impeller                            [m/s]

# Impeller hydraulic diameter (average of inlet and outlet, annular-passage approximation)
        Dh_imp_1   = D_1_t-D1h                                                                                                 # Inlet impeller hydraulic diameter                                        [m]
        Dh_imp_2   = 2*b2                                                                                                      # Exit impeller hydraulic diameter                                         [m]
        Dh_imp     = (Dh_imp_1+Dh_imp_2)/2                                                                                     # Average impeller hydraulic diameter                                      [m]

# Impeller friction coefficient (Colebrook equation, solved iteratively)
        rough      = 0.00005                                                                                                   # Impeller passage surface roughness                                       [m]
        rho_bar    = (rho_1+rho_2)/2                                                                                           # Impeller average density                                                 [kg/m^3]
        Re_imp     = rho_bar*w_bar*Dh_imp/Mu                                                                                   # Impeller passage Reynolds number                                         [-]
        cf_imp_n   = 0.02                                                                                                      # Value of first attempt for impeller friction coefficient                 [-]
        Error_cf   = 1                                                                                                         # Impeller friction coefficient error                                      [-]
        while Error_cf > 0.0001:
                cf_imp     = cf_imp_n                                                                                          # Impeller friction coefficient                                            [-]
                cf_imp_n   = 1/(-2*math.log10(rough/(3.71*Dh_imp)+2.51/(Re_imp*sqrt(cf_imp))))**2                              # New value of impeller friction coefficient                               [-]
                Error_cf   = abs(cf_imp_n-cf_imp)/cf_imp                                                                       # Impeller friction coefficient error                                      [-]
        cf_imp     = cf_imp_n                                                                                                  # Impeller friction coefficient (converged)                                [-]

        deltah_sf  = 2*cf_imp*(Lb/Dh_imp)*w_bar**2                                                                             # Impeller friction loss                                                   [J/kg]
   
    
#Impeller Recirculation
        deltah_rc   = 0.02*tan(alpha2)*(Df**2)*(u_2**2)                                                                        # Impeller recirculation loss (Coppage)                                    [J/kg]
...
#Impeller Leakage
        ...
        delta_PL = m_f*(r_2*cth2) / (Neff*r_bar*b_bar)         # Pressure difference at impeller tip   [Pa]
        u_l       = 0.816*sqrt(2*delta_PL/rho_2)                            # Leakage peripheral velocity           [m/s]
        delta_rad = 0.001*D_2                                               # Leakage clearance                     [m]
        m_fl      = rho_2*Neff*delta_rad*u_l*b_bar                          # Leakage mass flow rate                [kg/s]
...
#Diffuser Friction
        deltah_df = Cf * (r_4/r_2 - 1) * (r_2 / (b2 * cos(alpha2))) * c_2**2                                                         # Diffuser friction loss                                                   [J/kg]


#Vaneless Diffuser Loss (Station 2 -> Station 3)
        r_3        = 1.10*r_2                                                                                                    # Approximate radius at station 3                                          [m]
        D_3        = 2*r_3                                                                                                       # Diameter at station 3                                                    [m]
        k_vld      = 0.01                                                                                                        # Vaneless diffuser friction constant (typical)                            [-]
        Re_vld     = rho_2*c_2*(r_3-r_2)/Mu                                                                                       # Vaneless diffuser Reynolds number                                        [-]
        cf_vld     = k_vld*(1.8*10**5/Re_vld)**0.2                                                                                # Vaneless diffuser friction coefficient                                   [-]
        c_theta3   = cth2/(D_3/D_2 + (0.5*pi*cf_vld*rho_2*cth2*D_3*(D_3-D_2))/m_f)                                                # Tangential velocity at station 3                                        [m/s]
        deltah_vld = (cf_vld*r_2*(1-(r_2/r_3)**1.5)*c_2**2) / (1.5*b2*cos(alpha2))                                                # Vaneless diffuser friction loss                                         [J/kg]
...
#=======================================================================#        
        ...
        if i < number_stage:
                tot_loss       = delta_hbl+delta_hcl+deltah_df+deltah_vld+delta_hmx+deltah_sf+deltah_rc+deltah_lk+deltah_disk+delta_h_0_inc+delta_hin             # Total loss                           [J/kg]
        else:
                tot_loss       = delta_hbl+delta_hcl+deltah_df+deltah_vld+delta_hmx+deltah_sf+deltah_rc+deltah_lk+deltah_disk+delta_h_0_inc+delta_hin+delta_mer   # Total loss                           [J/kg]
...
#===============================RESULTS=================================#
    ...
    Ptot_tot_list.append(p_04/p_01)
...
#=======================================================================#
# Overall Isentropic vs Polytropic Efficiency Reconciliation (Reheat Effect)
#=======================================================================#

PR_overall = 1
for _v in Ptot_tot_list:
    PR_overall *= _v                                                          # Overall pressure ratio (product of per-stage Ptot_tot)   [-]

# Theoretical overall isentropic efficiency implied by a CONSTANT polytropic
# efficiency (Eta_p) across all stages, accounting for the reheat effect:
Eta_is_overall_from_poly = (PR_overall**((k-1)/k) - 1) / (PR_overall**((n-1)/n) - 1)

# Actual overall isentropic efficiency, from total real work vs total ideal
# (isentropic) work across the whole machine:
h_ideal_overall = c_p*T_01_list[0]*((PR_overall**((k-1)/k))-1)                  # Ideal (isentropic) enthalpy rise for the overall pressure ratio   [J/kg]
h_real_overall  = sum(Pot_list)*1000/m_f                                        # Actual total enthalpy rise, from total power and mass flow        [J/kg]
Eta_is_overall_actual = h_ideal_overall/h_real_overall

print("")
print("=== Overall Isentropic vs Polytropic Efficiency Check ===")
print(f"Assumed polytropic efficiency (Eta_p):                  {Eta_p:.4f}")
print(f"Overall pressure ratio achieved:                        {PR_overall:.3f}")
print(f"Overall isentropic efficiency implied by Eta_p (reheat effect):   {Eta_is_overall_from_poly:.4f}")
print(f"Overall isentropic efficiency, actual (total real vs total ideal work):   {Eta_is_overall_actual:.4f}")
print(f"Difference:                                             {abs(Eta_is_overall_from_poly-Eta_is_overall_actual):.4f}")
