...
Df_list = [] # Diffusion Factor [-]
Ptot_tot_list=[] # Total Pressure to Pressure Ratio []
beta_tt_imp_list = []   # Volumetric Flow Rate [-]
M_1_t_n_list = []
r_1_t_n_list = []
#=======================================================================#
# Input
...
# Shaft diameter based on preliminary criterion   
        ...
        M_1_t      = 4                                                          # Mach Number of the Inlet at the Tip (target/start)         [-]
        r_1_t_n    = r_1_h+0.35                                                 # Value of first attempt for impeller tip radius (safely above the Mach-minimizing point for every stage)   [m]
        ...
        while r_1_t_n > r_1_h:
                ...
                if M_1_t_n < M_1_t:
                        r_1_t_n   = r_1_t-0.001                                  # New value of tip radius (stepping down toward the Mach minimum)   [m]
...
#=======================================================================#        
    ...
    print(f"Stage {i}: tot_loss = {tot_loss:9.2f} J/kg | Eta_is = {Eta_is_n:.4f}")

    if i == 1:
#=======================================================================#
# Diagnostic: M_1r,tip vs D_1,tip sweep for the current stage
#=======================================================================#
        def M1t_of_D1t(D1t_try):
            r1t_try  = D1t_try/2
            rho_1n_l = p_01/(T_01*R_specific)
            Error_d_l = 1
            while Error_d_l > 0.0001:
                rho_1_l   = rho_1n_l
                A1_l      = pi*(r1t_try**2 - r_1_h**2)
                C_1_l     = m_f/(rho_1_l*A1_l)
                h_1_l     = h_01 - 0.5*C_1_l**2
                T_1_l     = h_1_l/c_p
                p_1_l     = p_01 - 0.5*rho_1_l*C_1_l**2
                rho_1n_l  = p_1_l/(T_1_l*R_specific)
                Error_d_l = abs(rho_1n_l-rho_1_l)/rho_1_l
            u_1t_l = Omega*r1t_try
            w_1t_l = sqrt(C_1_l**2+u_1t_l**2)
            a_1t_l = sqrt(k*R_specific*T_01)
            return w_1t_l/a_1t_l

        # Sweep range: stay above ~2*r_1_h + safety margin, since the density
        # iteration diverges (gives nonphysical results) at very small D_1,tip
        D_start = 2*r_1_h + 0.28
        D_range = [D_start + 0.002*j for j in range(50)]
        M_range = [M1t_of_D1t(D) for D in D_range]

        idx_min = M_range.index(min(M_range))
        print(f"Stage {i} diagnostic: Minimum M_1r,tip = {M_range[idx_min]:.4f} at D_1,tip = {D_range[idx_min]:.4f} m")

        plt.figure(figsize=(6,4))
        plt.plot(D_range, M_range)
        plt.axvline(D_range[idx_min], color='r', linestyle='--')
        plt.xlabel('D_1,tip [m]')
        plt.ylabel('M_1r,tip [-]')
        plt.title(f'Stage {i}: M_1r,tip vs D_1,tip')
        plt.grid(True)
        plt.show()
...
#===============================RESULTS=================================#
    ...
    Df_list.append(Df)
    Ptot_tot=p_04/p_01
    Ptot_tot_list.append(Ptot_tot)
    beta_tt_imp_list.append(beta_tt_imp)
    M_1_t_n_list.append(M_1_t_n)
    r_1_t_n_list.append(r_1_t_n)
