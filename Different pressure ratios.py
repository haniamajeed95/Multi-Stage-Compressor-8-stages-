...
p_07_list     = []   # Exit stage total pressure list      [Pa]
...
p_07       = 1.03*10**5                                                  # Inlet impeller total pressure      [Pa]
T_07       = 293.00                                                      # Inlet impeller total temperature   [K]
...
while i <= number_stage:
    ...
    p_01       = p_07                                                        # Inlet impeller total pressure                              [Pa]
    T_01       = T_07                                                        # Inlet impeller total temperature                           [K]
    ...
    Eta_is_n   = 0.8                                                         # Value of first attempt for isentropic efficiency           [-]
    beta_tt    = 80**0.175                                                   # Total pressure ratio of one stage                          [-]
    N_s        = 0.10                                                        # Specific speed                                             [-]
    Mu         = 1.789*10**(-5)                                              # Dynamic viscocity                                          [Pa.s]
    h_01       = c_p*T_01                                                    # Inlet impeller total enthalpy                              [J/kg]
    phi_2      = 0.22                                                        # Exit flow coefficient                                      [-]
    beta_2_inf = 50                                                          # Exit impeller relative blade angle (relative to radial)    [degree]
...
#=======================================================================#
    ...
    while Error_eta > 0.001:
        ...
        p_07   = p_01*beta_tt   # Exit return channel total pressure   [Pa]
        ...
        wth2       = u_2-cth2                                                    # TExit impeller tangential component of relative velocity   [m/s]
...
# Shaft diameter based on preliminary criterion   
        ...
        M_1_t      = 7                                                          # Mach Number of the Inlet at the Tip                        [-]
        r_1_h      = D1h/2                                                      # Hub Radius at the Inlet                                    [m]
        if i == 1:
            D_1_t_n    = D1h+0.3                                                # Value of first attempt for impeller tip diameter           [m]
        elif i == 3:
            D_1_t_n    = D1h+0.3                                                # Value of first attempt for impeller tip diameter           [m]
        elif i == 5:
            D_1_t_n    = D1h+0.1                                                # Value of first attempt for impeller tip diameter           [m]
        elif i == 7:
            D_1_t_n    = D1h+0.1                                                # Value of first attempt for impeller tip diameter           [m]
        r_1_t_n    = D_1_t_n/2                                                  # Value of first attempt for impeller tip radius             [m]
        ...
        beta_tt_imp   = (delta_h_0/(c_p*T_01)+1)**(k/(k-1))   # Impeller total to total pressure ratio   [-]   
...
#Impeller Recirculation
        deltah_rc   = 8*10**(-5)*sinh(3.5*alpha2**3)*(Df*u_2)**2                                                               # Impeller recirculation loss                                              [J/kg]
...
#===============================RESULTS=================================#
    ...
    p_2_list.append(p_2)
    T_2_list.append(T_2)
    h_2   = T_2/c_p   # Exit impeller temperature   [K]
    h_2_list.append(h_2)
    rho_2_list.append(rho_2)
    
    
    T_04   = h_04/c_p   # Exit diffuser temperature   [K]
...
#=======================================================================#
    ...
    p_01       = p_07                                                        # Inlet impeller total pressure                              [Pa]
    T_07       = T_04                                                        # Exit return channel total temperature                      [K]
    T_01       = T_07                                                        # Inlet impeller total temperature                           [K]
    beta_tt    = 80**0.075                                                   # Total pressure ratio of one stage                          [-]
    rho_01     = p_01/ (R_specific* T_01)                                    # Inlet impeller total density                               [kg/m^3]
    Eta_is_n   = 0.80                                                        # Value of first attempt for isentropic efficiency           [-]
    h_01       = c_p*T_01                                                    # Inlet impeller total enthalpy                              [J/kg]
    beta_2_inf = 50                                                          # Exit impeller relative blade angle (relative to radial)    [degree]
    ...
    while Error_eta > 0.001:
        ...
        p_07   = p_01*beta_tt   # Exit volute total pressure   [Pa]
        ...
        slip_f     = 1 - (cos(radians(beta_2_inf)) ** 0.5) / (Z ** 0.7)          # Slip factor                                                [-]
        phi_2      = (slip_f-tau)/tan(radians(beta_2_inf))                       # Exit flow coefficient                                      [-]
        cr2        = phi_2*u_2                                                   # Exit impeller meridional component of absolute velocity    [m/s]                                                                                                               [m/s]
        cth2       = tau*u_2                                                     # Exit impeller tangential component of absolute velocity    [m/s]
        wth2       = u_2-cth2                                                    # TExit impeller tangential component of relative velocity   [m/s]
        w2         = (cr2**2+wth2**2)**0.5                                       # Exit impeller relative velocity                            [m/s]
        ...
        w_1_h      = sqrt(u_1_h**2+C_1_m**2)   # Inlet impeller hub relative velocity   [m/s]
        beta_1_h   = math.acos(C_1_m/w_1_h)    # Inlet impeller hub relative flow angle   [rad.]
        beta_1_t   = math.acos(C_1_m/w_1_t)    # Inlet impeller tip relative flow angle   [rad.]ù
...
# Volute meridional component of absolute velocity loss
        delta_merid   = (1/2)*c_4r**2                                                                                           # Volute exit diffuser meridional component of absolute velocity loss     [J/kg]
...
#=======================================================================#        
        ...
        delta_h_0_id   = c_p * T_01 * ((beta_tt**((k - 1) / k)) - 1) / 1                                                   # Ideal impeller total enthalpy drop   [J/kg]
        tot_loss       = delta_hbl+delta_hcl+deltah_df+delta_hmx+deltah_sf+deltah_rc+deltah_lk+deltah_disk+delta_merid     # Total loss                           [J/kg]
        delta_h_real   = delta_h_0_id+tot_loss                                                                             # Real impeller total enthalpy drop    [J/kg]
        Eta_is_n       = delta_h_0_id/delta_h_real                                                                         # New value of isentropic efficiency   [-]
        Error_eta      = abs(Eta_is_n-Eta_is)/Eta_is                                                                       # Isentropic efficiency error          [-]
        # print(Error_eta,i)
